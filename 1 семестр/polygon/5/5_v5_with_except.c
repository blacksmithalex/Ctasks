...
double find_best_y(double x, Point *polygon, Line *lines, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (x1 <= x + EPS && x - EPS <= x2) {
            double denom = (x2 - x1);
            if (fabs(denom) < EPS) continue;  // Избегаем деления на ноль

            double y = y1 + (x - x1) * (y2 - y1) / denom;
            if (y < lower_y) lower_y = y;
            if (y > upper_y) upper_y = y;
        }
    }

    if (lower_y == INF || upper_y == -INF) return 0; // Ошибка: границы не найдены
...
double find_inscribed_radius(Point *polygon, Line *lines, int n) {
    ...
    if (fabs(left_x - right_x) < EPS) return 0; // Ошибка: X-граничные условия неверны
